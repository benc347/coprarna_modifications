...
my $intarna_pars = ""; ########################## BTC modification
my $run_code = ""; ########################## BTC modification
...
GetOptions ( ## edit 2.0.4
	...
	'intarna_file:s'=> \$intarna_pars, ############## BTC modification - text file containing additional intarna parameters
	'runname:s'		=> \$run_code, ############## BTC modification - string with a code to distinguish the kegg2refseq file for this run
);
...
foreach(@splitHeaderIDs) {
    ...
    #$_ =~ s/^\s+|\s+$//g; 
    my $availabilityCheck = `grep '$_' $PATH_COPRA/coprarna_aux/${run_code}/kegg2refseqnew.csv`;
...
########### BTC modification: check intarna parameters and add run code###########

my $inta_contents = `sed -n '1p' $intarna_pars`;
unless ($inta_contents) {
	$inta_contents = "";
}
print "intarna params: $inta_contents";
...
open WRITETOOPTIONS, ">", "CopraRNA_option_file.txt";
	...
	print WRITETOOPTIONS "additional intarna params:" . $inta_contents . "\n"; ###### BTC modification
	print WRITETOOPTIONS "run code:" . $run_code . "\n"; ###### BTC modification
...
if ($verbose) {
    ...
    chomp $ooi_rid;
    my $full_ooi = `grep '$ooi_rid' "$PATH_COPRA/coprarna_aux/${run_code}/CopraRNA_available_organisms.txt"`;
    chomp $full_ooi;
