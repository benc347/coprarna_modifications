...
################## BTC EDIT to add run code to kegg2refseq.csv location

# get run code
my $run_code = `grep 'run code:' CopraRNA_option_file.txt | sed 's/run code://g'`;
chomp $run_code;
...
################## BTC EDIT - added run code to file path of kegg2refseq.csv
my $keggtorefseqnewfile = $PATH_COPRA_SUBSCRIPTS . "${run_code}/kegg2refseqnew.csv";
...
print $PATH_COPRA_SUBSCRIPTS . "mod_parse_16s_from_gbk.pl $GenBankFiles > 16s_sequences.fa\n" if ($verbose);
system $PATH_COPRA_SUBSCRIPTS . "mod_parse_16s_from_gbk.pl $GenBankFiles > 16s_sequences.fa" unless (-e "16s_sequences.fa");
...
print $PATH_COPRA_SUBSCRIPTS . "mod_prepare_intarna_out.pl $ncrnas $upfromstartpos $down $mrnapart $GenBankFiles\n" if ($verbose);
system $PATH_COPRA_SUBSCRIPTS . "mod_prepare_intarna_out.pl $ncrnas $upfromstartpos $down $mrnapart $GenBankFiles";
## end  edit 2.0.0
...
# make annotations
system $PATH_COPRA_SUBSCRIPTS . "mod_annotate_raw_output.pl CopraRNA1_with_pvsample_sorted.csv opt_tags.clustered_rcsize $GenBankFiles > CopraRNA1_anno.csv" if ($cop1); ## edit 2.0.6
system $PATH_COPRA_SUBSCRIPTS . "mod_annotate_raw_output.pl CopraRNA2_prep_sorted.csv opt_tags.clustered $GenBankFiles > CopraRNA2_prep_anno.csv"; ## edit 2.0.6
...
unless ($cop1) {
    ...
    # run position script
    system "cp " . $PATH_COPRA_SUBSCRIPTS . "${run_code}/CopraRNA_available_organisms.txt ."; ## edit 2.0.6
...
unless ($cop1) {
    ...
    system "rm ${run_code}/CopraRNA_available_organisms.txt"; ## edit 2.0.6
}
