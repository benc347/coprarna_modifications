...
open(MYDATA, $treefile) or die("\nError: cannot open file $treefile at mod_compute_weights.pl\n\n");
    my @treefilelines = ();
...
open(MYDATA, $fneighfile) or die("\nError: cannot open file $fneighfile at mod_compute_weights.pl\n\n");
    my @fneighfilelines = ();
...
sub subtreelength 
{
    ...
    for my $key (keys %{$localhash{$nodenum}}) {
        if($key =~ m/[[:alpha:]]/) { ## edit 2.0.2 ## if an internal node points at a leaf we are done
